...
-(void) packetParser:(uint8_t *)cleanedMessage withSize:(ssize_t) n {
    ...
    for(int y = 0; y<n; y++) //THIS IS 1 FULL DECODABLE MESSAGE
    {
    ...
    switch (MID) {
        ...
        case 0x4C: //received iLevil message
            if (cleanedMessage[2] == 0x00)
            {
                ...
                //
                //                            int errorCode = cleanedMessage[6]*256+cleanedMessage[6];// refer to iLevil msg types for information on this
...
//main non-blocking msg receiver
- (void) msgReceiver
{
    self.errorInReceiving = false;
    //variables
    ssize_t n = 0;
    socklen_t len;
    ...
    //time struct for use in select statement
    struct timeval timeOutForRecvfrom;
    timeOutForRecvfrom.tv_sec = 0;
    timeOutForRecvfrom.tv_usec = 300000;
    
    //socket and sockaddr_in setting
    ...
    //variables for making recvfrom call timed
    int connectionStatus = 1;
    fd_set fdForDataAvailble;
    //clear the descriptor set
    FD_ZERO(&fdForDataAvailble);
    FD_SET(self.sockfd, &fdForDataAvailble);
    //int select (int nfds, fd_set *read-fds, fd_set *write-fds, fd_set *except-fds, struct timeval *timeout)
    connectionStatus = select(FD_SETSIZE, &fdForDataAvailble, NULL, NULL, &timeOutForRecvfrom);
    if (connectionStatus < 0)
    {
        ...
        if (FD_ISSET(self.sockfd, &fdForDataAvailble)){
        ...
        printf("\nAHRS Connection Timed out\n");
        self.errorInReceiving = true;
        printf("\nNOT CONNECTED/ POOR CONNECTION/ GENERATE NOT CONNECTION POP UP\n");
    }
    printf("\nreceiver function ended\n");
    //close(self.sockfd); //this is done in a separate function instead.
...
- (void) normalMsgSender: (NSString *) simulationMsgBytes ofSize:(uint8_t)numberOfBytes
{
    ...
    normalSvr.sin_addr.s_addr = inet_addr("192.168.1.11");
...
- (void) emergencyMsgSender:(NSString *) emgMsgBytes ofSize:(uint8_t) noOfBytes{
    ...
    emergencysvr.sin_addr.s_addr = inet_addr("192.168.1.11"); // SET THIS TO BE THE IP ADDRESS OF MICROCONTROLLER (192.168.1.11). DO NOT FORGET!
